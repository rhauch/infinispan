...
/**
 * A utility class for working with JSON documents. This class is able to read and write JSON documents that are in a
 * special modified format.
 * 
 * <h3>Modified Format</h3> Any JSON document written in this modified format is still a valid JSON document. However,
 * certain value types supported by BSON are written as nested objects in particular patterns. In fact, it is nearly
 * identical to the JSON serialization used by MongoDB. All standard JSON values are written as expected, but the types
 * unique to BSON are written as follows:
 * <p>
...
public class Json {
    ...
    * <p>
    * This method will read standard JSON and modified JSON, and tolerates whitespace and use of several delimeters,
    * including the standard ':' as well as '=' and '=>'.
    * </p>
    * 
    * 
    * @param url
    ...
    * 
    * @param json
    *           the JSON document string; may not be null
    * @return the in-memory {@link Document} representation
    * @throws ParsingException
    *            if there was a problem reading from the stream
    */
   ...
   /**
    * Return the modified JSON representation for the supplied in-memory {@link Document}. The resulting JSON will have
    * no embedded line feeds or extra spaces.
    * <p>
    * This format is compact and easy for software to read, but usually very difficult for people to read anything but
   ...
   /**
    * Return the modified JSON representation for the supplied object value. The resulting JSON will have no embedded
    * line feeds or extra spaces.
    * <p>
    * This format is compact and easy for software to read, but usually very difficult for people to read anything but
    * very small documents.
    * </p>
   ...
   /**
    * Write to the supplied writer the modified JSON representation of the supplied in-memory {@link Document}. The
    * resulting JSON will have no embedded line feeds or extra spaces.
    * <p>
    * This format is compact and easy for software to read, but usually very difficult for people to read anything but
    * very small documents.
    * </p>
   ...
   /**
    * Return the modified JSON representation for the supplied in-memory {@link Document}. The resulting JSON will be
    * indented for each name/value pair and each array value.
    * <p>
   ...
   /**
    * Return the modified JSON representation for the supplied object value. The resulting JSON will be indented for
    * each name/value pair and each array value.
    * <p>
    * This format is very readable by people and software, but is less compact due to the extra whitespace.
    * </p>
    * 
   ...
   /**
    * Write to the supplied writer the modified JSON representation of the supplied in-memory {@link Document}. The
    * resulting JSON will be indented for each name/value pair and each array value.
    * <p>
    * This format is very readable by people and software, but is less compact due to the extra whitespace.
    * </p>
    * 
