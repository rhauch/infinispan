...
public interface EditableArray extends EditableDocument, Array, List<Object> {
    ...
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @param value
    *           the new value for the field
    * @return This array, to allow for chaining methods
    */
   ...
   /**
    * Set the value for the field at the given index to a {@link Symbol} created from the supplied string value. Symbols
    * are defined in the BSON specification as being similar to a string but which exists for those languages that have
    * a specific symbol type. Symbols are serialized to JSON as a normal string.
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @param value
    ...
    * @param document
    *           the document
    * @return The editable document that was just set as the value for the named field; never null and may or may not be
    *         the same instance as the supplied <code>document</code>.
    */
    ...
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @return The editable document field value, if found, or null if there is no such pair or if the value is not a
    *         document
    */
    ...
    * @param array
    *           the array
    * @return The editable array that was just set as the value for the named field; never null and may or may not be
    *         the same instance as the supplied <code>array</code>.
    */
    ...
    * <ul>
    * <li>"<code><i>yyyy</i>-<i>MM</i>-<i>dd</i>T<i>HH</i>:<i>mm</i>:<i>ss</i></code>" where "<code>T</code>" is a
    ...
    *            if the supplied value could not be parsed into a valid date
    */
   EditableArray setDate(String name, String isoDate) throws ParseException;

   /**
    * Set the value for the field at the given index to a {@link Timestamp} with the supplied time in seconds and
    * increment. Note that {@link Date} values are recommended for most purposes, as they are better suited to most
    * applications' representations of time instants.
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @param timeInSeconds
    *           the time in seconds for the new Timestamp
    * @param increment
   ...
   /**
    * Set the value for the field at the given index to an {@link ObjectId} created from the supplied hexadecimal binary
    * value. Object IDs are defined by the BSON specification as 12-byte binary values designed to have a reasonably
    * high probability of being unique when allocated. Since there is no explicit way to represent these in a JSON
    * document, each ObjectId value is serialized in a JSON document as a nested document of the form:
    * 
    * <pre>
    * { "$oid" : "<i>12bytesOfIdInBase16</i>" }
    * </pre>
    * 
    * When nested documents of this form are read by this library's {@link Json JSON reader}, nested documents of this
    * form will be converted to an ObjectId value.
    * <p>
    * For example, an ObjectId with time value of "1310745823", machine value of "1", process value of "2", and
    * increment value of "3" would be written as
    * 
    * <pre>
   ...
   /**
    * Set the value for the field at the given index to an {@link ObjectId} created from the supplied 12-byte binary
    * value. Object IDs are defined by the BSON specification as 12-byte binary values designed to have a reasonably
    * high probability of being unique when allocated. Since there is no explicit way to represent these in a JSON
    * document, each ObjectId value is serialized in a JSON document as a nested document of the form:
    * 
    ...
    *           The name of the field, which is the string representation of the index in the array
    * @param time
    *           the Unix-style timestamp, which is a signed integer representing the number of seconds before or after
    *           January 1st 1970 (UTC)
    * @param machine
    *           the first three bytes of the (md5) hash of the machine host name, or of the mac/network address, or the
    *           virtual machine id
    * @param process
    *           the 2 bytes of the process id (or thread id) of the process generating the object id
    * @param inc
    ...
    * represented in memory using {@link Pattern} instances, and are stored natively in BSON as regular expressions.
    * However, when serialized to JSON, regular expressions are written as nested documents of the form:
    * 
    * <pre>
    * { "$regex" : "<i>pattern</i>" }
    * </pre>
    ...
    * <p>
    * When nested documents of this form are read by this library's {@link Json JSON reader}, nested documents of this
    * form will be converted to a regular expression value.
    * </p>
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    ...
    * </pre>
    * 
    * where "<i>pattern</i>" is the regular expression pattern, and "<i>flags</i>" is a string representation of the
    ...
    *           the regular expression pattern string
    * @param flags
    *           the bitwise-anded {@link Pattern} options: {@link Pattern#CANON_EQ}, {@link Pattern#CASE_INSENSITIVE},
    *           {@link Pattern#CASE_INSENSITIVE}, {@link Pattern#COMMENTS}, {@link Pattern#DOTALL},
    *           {@link Pattern#LITERAL}, {@link Pattern#MULTILINE}, {@link Pattern#UNICODE_CASE}, and
   ...
   /**
    * Set the value for the field at the given index to be a null value. Both JSON and BSON formats support null values,
    * and {@link Null} is used for the value in the in-memory representation. The {@link #isNull(String)} methods can be
    * used to determine if a field has been set to null, or {@link #isNullOrMissing(String)} if the field has not be set
    * or if it has been set to null.
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @return This document, to allow for chaining methods
   ...
   /**
    * Set the value for the field at the given index to be a binary value. JSON does not formally support binary values,
    * and so such values will be encoded using a nested document of the form:
    * 
    * <pre>
    * { "$type" : <i>typeAsInt</i>, "$base64" : "<i>bytesInBase64</i>" }
    * </pre>
    * 
    * where "<i>typeAsInt</i>" is the integer representation of the {@link BinaryType BSON type}, and
    * "<i>bytesInBase64</i>" is the Base64 encoding of the actual Binary {@link Binary#getBytes() bytes}.
    * <p>
    * When nested documents of this form are read by this library's {@link Json JSON reader}, nested documents of this
    * form will be converted to Binary value.
    * </p>
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
    * @param type
    *           one of the {@link BinaryType BSON type} constants denoting the type of the {@link Binary} value
    * @param data
    *           the bytes for the {@link Binary} value
    * @return This document, to allow for chaining methods
    */
   EditableArray setBinary(String name, byte type, byte[] data);

   /**
    * Set the value for the field at the given index to be a {@link UUID}. JSON does not formally support binary values,
    * and so such values will be encoded using a nested document of the form:
    * 
    * <pre>
    ...
    * <p>
    * When nested documents of this form are read by this library's {@link Json JSON reader}, nested documents of this
    * form will be converted to UUID value.
    * </p>
    * 
    * @param name
    *           The name of the field, which is the string representation of the index in the array
   ...
   /**
    * Set the value for the field at the given index to be a {@link Code} or {@link CodeWithScope}. JSON does not
    * formally support such values, and so when written to JSON they will be encoded using a nested document of the
    * form:
    * 
    * <pre>
    * { "$code" : "<i>code</i>" }
    ...
    * </pre>
    * 
    * where "<i>code</i>" is the {@link Code}'s {@link Code#getCode() JavaScript code} and <i>scopeDocument</i> is the
    * nested document representing the {@link CodeWithScope#getScope() scope} in which the JavaScript code should be
    * evaluated.
    * <p>
    * When nested documents of this form are read by this library's {@link Json JSON reader}, nested documents of this
    * form will be converted to {@link Code} or {@link CodeWithScope} value.
    * </p>
    * <p>
    * Note that when <code>includeScope</code> is <code>true</code>, the returned {@link EditableArray} can be used to
    ...
    *           the code
    * @param includeScope
    *           true if the code should include a scope (and if this method should return an {@link EditableArray} for
    *           this scope document), or false otherwise
    * @return if <code>includeScope</code> is <code>true</code>, then the {@link EditableDocument} for the scope;
    *         otherwise, this array to allow for chaining methods
    ...
    * @param document
    *           the document
    * @return The editable document that was just set as the value at the supplied index in this array; never null and
    *         may or may not be the same instance as the supplied <code>document</code>.
    */
    ...
    * @param array
    *           the array
    * @return The editable array that was just set as the value at the supplied index in this array; never null and may
    *         or may not be the same instance as the supplied <code>array</code>.
    */
    ...
    *            if the supplied value could not be parsed into a valid date
    */
   EditableArray setDate(int index, String isoDate) throws ParseException;
   ...
   /**
    * Insert the value for the field at the given index to a {@link Symbol} created from the supplied string value.
    * Symbols are defined in the BSON specification as being similar to a string but which exists for those languages
    * that have a specific symbol type. Symbols are serialized to JSON as a normal string.
    * 
    * @param index
    *           The index in the array at which the value is to be set
    * @param value
    ...
    *            if the supplied value could not be parsed into a valid date
    */
   EditableArray addDate(int index, String isoDate) throws ParseException;

   /**
    * Insert the value for the field at the given index to a {@link Timestamp} with the supplied time in seconds and
    * increment. Note that {@link Date} values are recommended for most purposes, as they are better suited to most
    * applications' representations of time instants.
    * 
    * @param index
   ...
   /**
    * Insert the value for the field at the given index to an {@link ObjectId} created from the supplied hexadecimal
    * binary value. Object IDs are defined by the BSON specification as 12-byte binary values designed to have a
    * reasonably high probability of being unique when allocated. Since there is no explicit way to represent these in a
    * JSON document, each ObjectId value is serialized in a JSON document as a nested document of the form:
    * 
    * <pre>
    * { "$oid" : "<i>12bytesOfIdInBase16</i>" }
    * </pre>
   ...
   /**
    * Insert the value for the field at the given index to an {@link ObjectId} created from the supplied 12-byte binary
    * value. Object IDs are defined by the BSON specification as 12-byte binary values designed to have a reasonably
    * high probability of being unique when allocated. Since there is no explicit way to represent these in a JSON
    * document, each ObjectId value is serialized in a JSON document as a nested document of the form:
    * 
   ...
   /**
    * Insert the value for the field at the given index to the supplied regular expression. Regular expression values
    * are represented in memory using {@link Pattern} instances, and are stored natively in BSON as regular expressions.
    * However, when serialized to JSON, regular expressions are written as nested documents of the form:
    * 
    * <pre>
    * { "$regex" : "<i>pattern</i>" }
   ...
   /**
    * Insert the value for the field at the given index to be a null value. Both JSON and BSON formats support null
    * values, and {@link Null} is used for the value in the in-memory representation. The {@link #isNull(String)}
    * methods can be used to determine if a field has been set to null, or {@link #isNullOrMissing(String)} if the field
    * has not be set or if it has been set to null.
    * 
    * @param index
    *           The index in the array at which the value is to be set
    * @return This document, to allow for chaining methods
   ...
   /**
    * Insert the value for the field at the given index to be a binary value. JSON does not formally support binary
    * values, and so such values will be encoded using a nested document of the form:
    * 
    * <pre>
    * { "$type" : <i>typeAsInt</i>, "$base64" : "<i>bytesInBase64</i>" }
    * </pre>
   ...
   /**
    * Insert the value for the field at the given index to be a {@link UUID}. JSON does not formally support binary
    * values, and so such values will be encoded using a nested document of the form:
    * 
    * <pre>
    * { "$uuid" : "<i>string-form-of-uuid</i>" }
   ...
   /**
    * Insert the value for the field at the given index to be a {@link Code} or {@link CodeWithScope}. JSON does not
    * formally support such values, and so when written to JSON they will be encoded using a nested document of the
    * form:
    * 
    * <pre>
    ...
    * @param document
    *           the document
    * @return The editable document that was just added to this array; never null and may or may not be the same
    ...
    * @param array
    *           the array
    * @return The editable array that was just added to this array; never null and may or may not be the same instance
    *         as the supplied <code>array</code>.
    */
    ...
    *           true if the code should include a scope (and if this method should return an {@link EditableArray} for
    *           this scope document), or false otherwise
    * @return if <code>includeScope</code> is <code>true</code>, then the {@link EditableArray} for the scope;
    *         otherwise, this document to allow for chaining methods
   ...
   /**
    * Adds the supplied boolean value to this array, if and only if an equivalent value doesn't already exist in the
    * array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds the supplied integer value to this array, if and only if an equivalent value doesn't already exist in the
    * array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds the supplied float value to this array, if and only if an equivalent value doesn't already exist in the
    * array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds the supplied double value to this array, if and only if an equivalent value doesn't already exist in the
    * array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds the supplied string value to this array, if and only if an equivalent value doesn't already exist in the
    * array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds to this array a Symbol with the supplied string, if and only if an equivalent value doesn't already exist in
    * the array.
    * 
    * @param value
    *           the new value for the field
   ...
   /**
    * Adds to this array a Date with the supplied ISO-8601 string, if and only if an equivalent value doesn't already
    * exist in the array.
    * 
    * @param isoDate
    *           the new value for the field
   ...
   /**
    * Adds to this array a Timestamp with the supplied time in seconds and increment value, if and only if an equivalent
   ...
   /**
    * Adds to this array an ObjectId with the supplied hexadecimal string, if and only if an equivalent value doesn't
   ...
   /**
    * Adds to this array an ObjectId with the supplied 12-byte value, if and only if an equivalent value doesn't already
    * exist in the array.
    * 
    * @param bytes
    *           the 12-byte value for the ObjectId
   ...
   /**
    * Adds to this array an ObjectId with the supplied time, machine, process, and increment, if and only if an
    * equivalent value doesn't already exist in the array.
    * 
    * @param time
    *           the Unix-style timestamp, which is a signed integer representing the number of seconds before or after
    *           January 1st 1970 (UTC)
   ...
   /**
    * Adds to this array a regular expression with the supplied pattern string, if and only if an equivalent value
   ...
   /**
    * Adds to this array a regular expression with the supplied pattern string and option flags, if and only if an
    * equivalent value doesn't already exist in the array.
    * 
    * @param pattern
    *           the regular expression pattern string
   ...
   /**
    * Adds to this array a {@link Binary} value with the supplied type and content, if and only if an equivalent value
    * doesn't already exist in the array.
    * 
    * @param type
    *           one of the {@link BinaryType BSON type} constants denoting the type of the {@link Binary} value
   ...
   /**
    * Adds to this array a {@link CodeWithScope} with the supplied JavaScript code and scope, if and only if an
    * equivalent value doesn't already exist in the array.
    * 
    * @param code
    *           the code
