...
/**
 * A store for JSON documents and other binary content, plus a library of JSON Schema documents used to describe and
 * validate the stored documents.
 * 
...
public interface SchematicDb extends Lifecycle {
   ...
   /**
    * Validate all JSON documents stored within this database, using this database's {@link #getSchemaLibrary() JSON
    * Schema library}. This method works even when the database is distributed, and it blocks until the task is
    * completed.
    * 
    * @return the map of document keys to validation results for all of the JSON documents in the database that are
    *         affiliated with a schema and that had at least one validation error or warning
    */
    ...
    * {@link #getSchemaLibrary() JSON Schema library}. This method works even when the database is distributed, and it
    * blocks until the task is completed.
    * 
    * @param firstKey
    *           the first key of the document that is to be validated
    * @param additionalKeys
    *           the additional keys of the documents that are to be validated
    * @return the map of document keys to validation results for all of the JSON documents in the database that are
    *         affiliated with a schema and that had at least one validation error or warning
    */
   Map<String, Results> validate(String firstKey, String... additionalKeys);
    ...
    *           the key or identifier for the document
    * @param binaryContent
    *           the binary content that is to be stored
    * @param metadata
    *           the metadata that is to be stored; may be null if there is no metadata
   ...
   /**
    * Replace the existing document and metadata at the given key with the document that is supplied. This method does
    * nothing if there is not an existing entry at the given key.
    * 
    * @param key
    *           the key or identifier for the document
    * @param document
    *           the new document that is to replace the existing document (or binary content)
    * @param metadata
   ...
   /**
    * Replace the existing document and metadata at the given key with the document that is supplied. This method does
    * nothing if there is not an existing document at the given key.
    * 
    * @param key
    *           the key or identifier for the document
    * @param document
    ...
    * @param binaryContent
    *           the binary content that is to replace the existing binary content (or document)
    * @param metadata
    *           the metadata that is to be stored with the replacement content; may be null if there is no metadata for
    *           the replacement
    * @return the entry that was replaced, or null if nothing was replaced
    */
   ...
   /**
    * Asynchronous version of {@link #put(String, Document, Document)}. This method does not block on remote calls, even
    ...
    */
   NotifyingFuture<SchematicEntry> putAsync(String key, Document document, Document metadata);

   /**
    * Asynchronous version of {@link #put(String, Binary, Document)}. This method does not block on remote calls, even
    * if the library cache mode is synchronous.
    * 
    * @param key
    *           the key or identifier for the document
    ...
    * @return a future containing the old document that was previously stored at this key; never null
    */
   NotifyingFuture<SchematicEntry> putAsync(String key, Binary binaryContent, Document metadata);

   /**
    * Asynchronous version of {@link #putIfAbsent(String, Document, Document)}. This method does not block on remote
    * calls, even
    * 
    * @param key
    *           the key or identifier for the document
    ...
    * @param metadata
    *           the metadata that is to be stored; may be null if there is no metadata
    * @return a future containing the existing document, or the null if there was no existing document at the supplied
    *         key
    */
   NotifyingFuture<SchematicEntry> putIfAbsentAsync(String key, Document document, Document metadata);

   /**
    * Asynchronous version of {@link #putIfAbsent(String, Binary, Document)}. This method does not block on remote
    * calls, even
    * 
    * @param key
    *           the key or identifier for the document
    ...
    *         key
    */
   NotifyingFuture<SchematicEntry> putIfAbsentAsync(String key, Binary binaryContent, Document metadata);

   /**
    * Asynchronous version of {@link #replace(String, Document, Document)}. This method does not block on remote calls,
    * even if the library cache mode is synchronous.
    * 
    * @param key
    *           the key or identifier for the document
    * @param document
    ...
    */
   NotifyingFuture<SchematicEntry> replaceAsync(String key, Document document, Document metadata);

   /**
    * Asynchronous version of {@link #replace(String, Binary, Document)}. This method does not block on remote calls,
    * even if the library cache mode is synchronous.
    * 
    * @param key
    *           the key or identifier for the document
    * @param binaryContent
    *           the new binary content that is to replace the existing content (or document)
    * @param metadata
    *           the metadata that is to be stored with the replacement document; may be null if there is no metadata for
    *           the replacement
    * @return a future containing the entry that was replaced; never null
    */
   NotifyingFuture<SchematicEntry> replaceAsync(String key, Binary binaryContent, Document metadata);
