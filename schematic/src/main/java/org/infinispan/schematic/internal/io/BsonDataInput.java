...
/**
 * An implementation of {@link DataInput} with additional methods needed for reading BSON formatted content from another
 * DataInput instance. Specifically, this class reads little-endian byte order (purposefully in violation of the
 * DataInput interface specification) and provides a way to read C-style strings (where the length is not known up front
 * but instead contain all characters until the zero-byte terminator), which are commonly used within the BSON
 * specification.
 * 
...
public class BsonDataInput implements DataInput {
   ...
   /**
    * A thread-local cache of ByteBuffer and CharBuffer instances. Reusing these buffers results in fewer allocations
    * and less to garbage collect. Note that this cache has to be thread-local, since {@link BufferCache} is not
    * thread-safe.
    */
   ...
   /**
    * Read a UTF-8 string with the supplied length or, if the length is not known, the UTF-8 characters until the next
    * zero-byte value. Note that this is different than the {@link #readUTF() standard way} to read a string, which
    * always expects the length to be the first value on the stream.
    * 
    * @param len
    *           the number of bytes to read, or -1 if the length is not known and characters should be read until the
    *           next zero-byte string.
   ...
   /**
    * Utility method to read a UTF-8 string from the supplied DataInput object given the supplied number of bytes to
    * read or, if the number of bytes is not known, all of the UTF-8 characters until the next zero-byte value. Note
    * that this is different than the {@link #readUTF() standard way} to read a string, which always expects the length
    * to be the first value on the stream.
    * 
    * @param dis
    *           the DataInput from which the UTF-8 string is to be read
    * @param len
    *           the number of bytes to read, or -1 if the length is not known and characters should be read until the
    *           next zero-byte string.
    * @return the read UTF-8 string
