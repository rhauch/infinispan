...
public class CacheSchematicDb implements SchematicDb {
   ...
   @Override
   public Map<String, Results> validate(String firstKey, String... additionalKeys) {
   ...
   protected static class ResultsCollector implements Collector<String, Results> {
      ...
      @Override
      public void emit(String key, Results value) {
   ...
   public SchematicEntry putIfAbsent(String key, Document document, Document metadata) {
      ...
      newEntry.setContent(document, metadata, defaultContentTypeForDocument);
      SchematicEntry existingEntry = (SchematicEntryLiteral) store.putIfAbsent(key, newEntry);
   ...
   protected class SchematicEntryProxyFuture implements NotifyingFuture<SchematicEntry> {
      ...
      @Override
      public SchematicEntry get(long timeout, TimeUnit unit) throws InterruptedException, ExecutionException,
               TimeoutException {
