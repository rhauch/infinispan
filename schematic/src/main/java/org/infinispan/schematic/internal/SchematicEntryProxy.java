...
/**
 * A layer of indirection around an {@link SchematicEntryLiteral} to provide consistency and isolation for concurrent
 ...
 * {@link SchematicEntryLiteral#getProxy(Cache, String, BatchContainer, InvocationContextContainer)} should be used to
 * retrieve an instance of this proxy. <br />
 * <br />
 * Typically proxies are only created by the {@link SchematicEntryLookup} helper, and would not be created by end-user
 * code directly.
 * 
...
public class SchematicEntryProxy extends AutoBatchSupport implements SchematicEntry {
   ...
   SchematicEntryProxy(Cache<String, SchematicEntry> cache, String key, BatchContainer batchContainer,
   ...
   /**
    * Utility method that casts the supplied cache entry value to a {@link SchematicEntryLiteral literal value} for
    * reading purposes.
    * 
    * @param object
    *           the cache entry value, which may be a {@link MarshalledValue}
   ...
   private void assertValid(SchematicEntryLiteral value) {
      if (startedReadingValue && (value == null || value.removed))
         throw new IllegalStateException("SchematicValue stored under key " + key + " has been concurrently removed!");
   }

   /**
    * Utility method that gets the {@link SchematicEntryLiteral literal value} for this proxy, and which marks the value
    * for reading and performs various assertion checks. Comment this
    * 
    * @return
    */
   ...
   private SchematicEntryLiteral getDeltaValueForWrite(InvocationContext ctx) {
      ...
      boolean lockedAndCopied = lookedUpEntry != null && lookedUpEntry.isChanged()
               && toValue(lookedUpEntry.getValue()).copied;
