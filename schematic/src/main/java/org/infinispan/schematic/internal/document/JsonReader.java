...
import org.infinispan.schematic.internal.document.JsonReader.Parser.FunctionCall;
 ...
 * <p>
 * This reader is capable of optionally introspecting string values to look for certain string patterns that are
 * commonly used to represent dates. In introspection is not done by default, but when it is used it looks for the
 * following patterns:
 * <ul>
 ...
 * </pre>
 * 
 * where <code><i>parameters</i></code> consists of one or more JSON values (including nested functions). If the
 * function call cannot be parsed and executed, the string literal form of the function call is kept.
 * </p>
 * 
...
public class JsonReader {
   ...
   /**
    * Read the JSON representation from supplied input stream and construct the {@link Document} representation, using
    * the {@link Charset#defaultCharset() default character set}.
    * 
    * @param stream
   ...
   /**
    * Read the JSON representation from supplied input stream and construct the {@link Document} representation, using
    * the supplied {@link Charset character set}.
    * 
    * @param stream
    *           the input stream; may not be null
    * @param charset
    *           the character set that should be used; may not be null
    * @return the in-memory {@link Document} representation
    * @throws ParsingException
    *            if there was a problem reading from the stream
    */
    ...
    * 
    * @param json
    *           the JSON representation; may not be null
    * @return the in-memory {@link Document} representation
    * @throws ParsingException
    *            if there was a problem reading from the stream
    */
    ...
    * @param stream
    *           the input stream; may not be null
    * @param introspectStringValues
    *           true if the string values should be examined for common patterns, or false otherwise
    * @return the in-memory {@link Document} representation
    * @throws ParsingException
    *            if there was a problem reading from the stream
    */
   ...
   public static class Parser {
       ...
       * @param valueMatcher
       *           the component that looks for patterns within string values to create alternative objects; may not be
       *           null
       */
       ...
       * @return the document
       * @throws ParsingException
       *            if there is a problem parsing the value
       */
       ...
       * Parse the stream for the next JSON document.
       * 
       * @param hasSpecialNames
       *           the flag that should be set if this document contains field names that are reserved
       * @return the document
       * @throws ParsingException
       *            if there is a problem parsing the value
       */
      ...
      /**
       * Parse the JSON array on the stream, beginning with the '[' character until the ']' character, which is
       * consumed.
       * 
       ...
       * <li>a string literal date of the form
       * <code>"<i>yyyy</i>-<i>MM</i>-<i>dd</i>T<i>HH</i>:<i>mm</i>:<i>ss</i>"</code> where <code>T</code> is a literal
       * character</li>
       * <li>a string literal date of the form
       * <code>"<i>yyyy</i>-<i>MM</i>-<i>dd</i>T<i>HH</i>:<i>mm</i>:<i>ss</i>Z"</code> where <code>T</code> and
       ...
       * <li>a function of the form <code>new <i>functionName</i>(<i>parameters</i>)</code> where
       * <code><i>parameters</i></code> consists of one or more values as parsed by this method
       * </ul>
       * Note that in the date forms listed above, <code><i>millisOrIso</i></code> is either a long value representing
       * the number of milliseconds since epoch or a string literal in ISO-8601 format representing a date and time.
       * 
      ...
      /**
       * Parse the value given by the supplied string located at the supplied line and column numbers. This method looks
       * for known constant values, then attempts to parse the value as a number, and then calls
       * {@link #parseUnknownValue(String, int, int)}.
       * 
       * @param value
       *           the string representation of the value
       * @param lineNumber
       *           the line number for the beginning of the value
       * @param columnNumber
      ...
      /**
       * Parse the number represented by the supplied value. This method is called by the
       * {@link #parseValue(String, int, int)} method.
       * 
       * @param value
       *           the string representation of the value
       ...
       * @param columnNumber
       *           the column number at which the value starts
       * @return the value
       * @throws ParsingException
       *            if there is a problem parsing the value
       */
      ...
      public Object parseFunction() throws ParsingException {
         ...
         if (c != '(') {
            throw tokens.error("Expected '(' after function name \"" + functionName + "\" and at line "
                     + tokens.lineNumber() + ", column " + tokens.columnNumber());
         }
      ...
      /**
       * Method that is called to evaluate the supplied function. This method may be overridden by subclasses to handle
       * custom functions.
       * 
       * @param function
       *           the function definition
       * @return the value that resulted from evaluating the function, or null if the function call could not be
       *         evaluated
       * @throws ParsingException
       *            if there is a problem parsing the value
       */
      public Object evaluateFunction(FunctionCall function) throws ParsingException {
         ...
         if ("date".equalsIgnoreCase(function.getFunctionName())) {
            if (numParams > 0) {
               ...
               if (value instanceof String) {
                  ...
                  try {
                     ...
                     // Not a valid date ...
                     throw tokens
                              .error("Expecting the \"new Date(...)\" parameter to be a valid number of milliseconds or ISO date string, but found \""
                                       + param1.getValue()
                                       + "\" at line "
            ...
            // Not a valid date ...
            throw tokens.error("The date function requires one parameter at line " + function.getLineNumber()
                     + ", column " + function.getColumnNumber());
         }
         return null;
      }

      /**
       * Method that is called when the function call described by the parameter could not be evaluated. By default, the
       * string representation of the function is returned.
       * 
       * @param function
       *           the function definition
   ...
   /**
    * The component that matches a string value for certain patterns. If the value matches a known pattern, it return
    * the appropriate value object; otherwise, the supplied string value is returned.
    * 
    * @author Randall Hauch <rhauch@redhat.com> (C) 2011 Red Hat Inc.
    * @since 5.1
    */
   ...
   public static interface ValueMatcher {
      ...
      /**
       * Parse the value given by the supplied string into an appropriate value object. This method looks for specific
       * patterns of Date strings; if no known pattern is found, it just returns the supplied value.
       * 
       * @param value
       *           the string representation of the value
       * @return the value
   ...
   public static class DateValueMatcher extends SimpleValueMatcher {
       ...
       * {@link #parseValue(String, int, int)} method. This method checks the following formats:
       * <ul>
       * <li>a string literal date of the form
       * <code>"<i>yyyy</i>-<i>MM</i>-<i>dd</i>T<i>HH</i>:<i>mm</i>:<i>ss</i>"</code> where <code>T</code> is a literal
       * character</li>
       ...
       * </ul>
       * <p>
       * Note that this method does not handle the <code>new Date(...)</code> or <code>Date(...)</code> representations,
       * as that's handled by the {@link #evaluateFunction(FunctionCall)} method.
       * </p>
       * 
       * @param value
   ...
   public static class Tokenizer {
       ...
       * 
       * @param reader
       *           the reader for accessing the JSON content; may not be null
       */
      ...
      protected char next() throws ParsingException {
         ...
         if (hasPrevious) {
            ...
            try {
               ...
               throw error(
                        "Error reading at line " + lineNumber + ", column " + columnNumber + ": "
      ...
      /**
       * Read the next quoted string from the stream, where stream begins with the a single-quote or double-quote
       * character and the string ends with the same quote character.
       * 
      ...
      /**
       * Consume the next document delimiter (either a ',' or a ';'), and return whether the end-of-document character
       * (e.g., '}') has been consumed. This will correctly handle repeated delimiters, which are technically incorrect.
       * 
       * @return true if a '}' has been consumed, or false otherwise
       * @throws ParsingException
       *            if the document delimiter could not be read
       */
