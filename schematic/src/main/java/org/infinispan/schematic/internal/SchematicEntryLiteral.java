...
public class SchematicEntryLiteral implements SchematicEntry, DeltaAware {
    ...
    * CacheDelegate. User code should use {@link SchematicEntryLookup#getSchematicValue(Cache, String)}.
    * 
    * @param cache
    *           underlying cache
    * @param key
    ...
    */
   public SchematicEntry getProxy(Cache<String, SchematicEntry> cache, String mapKey, BatchContainer batchContainer,
            InvocationContextContainer icc) {
      // construct the proxy lazily
      if (proxy == null) { // DCL is OK here since proxy is volatile (and we live in a post-JDK 5 world)
         synchronized (this) {
            if (proxy == null)
               proxy = new SchematicEntryProxy(cache, mapKey, batchContainer, icc);
         }
      }
      return proxy;
   }
   ...
   protected Object setContent(Object content) {
      ...
      if (content != null) {
         ...
         getDelta().addOperation(new RemoveOperation(FieldPath.CONTENT, existing));
      }
      return existing;
   }
   ...
   protected void setMetadata(Document metadata, String defaultContentType) {
      if (metadata != null) {
         ...
         for (Field field : metadata.fields()) {
            ...
            if (fieldName.equals(FieldName.ID))
               continue;
