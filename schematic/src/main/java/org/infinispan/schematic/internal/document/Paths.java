...
public class Paths {
   ...
   private static final Path EMPTY_PATH = new EmptyPath();
   ...
   protected static final String notNullOrEmpty(String value, String name) {
      if (value == null)
         throw new IllegalArgumentException("The '" + name + "' argument may not be null");
      value = value.trim();
      if (value.length() == 0)
   ...
   public static class Externalizer extends AbstractExternalizer<Path> {
      ...
      @Override
      public Path readObject(ObjectInput input) throws IOException, ClassNotFoundException {
