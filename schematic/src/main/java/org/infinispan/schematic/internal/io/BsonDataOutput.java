...
/**
 * An implementation of {@link DataOuput} with additional methods needed for writing BSON formatted content.
 * Specifically, this class reads little-endian byte order (purposefully in violation of the DataInput interface
 * specification) and provides a way to read C-style strings (where the length is not known up front but instead contain
 * all characters until the zero-byte terminator), which are commonly used within the BSON specification.
 * 
...
public class BsonDataOutput implements DataOutput {
    ...
    * thrown.
    * <p>
    * If <code>s.length</code> is zero, then no bytes are written. Otherwise, the character <code>s[0]</code> is written
    * first, then <code>s[1]</code>, and so on; the last character written is <code>s[s.length-1]</code>. For each
    * character, one byte is written, the low-order byte, in exactly the manner of the <code>writeByte</code> method .
    ...
    * bytes are actually written, low-order byte first, in exactly the manner of the <code>writeChar</code> method.
    * 
    * @param s
    *           the string value to be written.
    * @throws IOException
   ...
   /**
    * Writes the <a href="DataInput.html#modified-utf-8">modified UTF-8</a> representation of every character in the
    * string <code>s</code>. <i>This is similar to the standard {@link #writeUTF(String)} but without the leading two
    * byte length.</i> If <code>s</code> is <code>null</code>, a <code>NullPointerException</code> is thrown. Each
    * character in the string <code>s</code> is converted to a group of one, two, or three bytes, depending on the value
    * of the character.
    * <p>
    * If a character <code>c</code> is in the range <code>&#92;u0001</code> through <code>&#92;u007f</code>, it is
    ...
    * </pre>
    * <p>
    * If a character <code>c</code> is in the range <code>&#92;u0800</code> through <code>uffff</code>, then it is
    ...
    * </pre>
    * <p>
    * First, the total number of bytes needed to represent all the characters of <code>s</code> is calculated. If this
    * number is larger than <code>65535</code>, then a <code>UTFDataFormatException</code> is thrown. Otherwise, this
    * length is written to the output stream in exactly the manner of the <code>writeShort</code> method; after this,
    * the one-, two-, or three-byte representation of each character in the string <code>s</code> is written.
    * <p>
         ...
         while (chars.hasRemaining()) {
            ...
            if (output == remainderBuffer) {
               ...
               while (output.remaining() > 0) {
                  writeByte(newPosition, (int) output.get());
                  ++newPosition;
               }
            } else {
    ...
    * @throws IOException
    *            if there is a problem writing to the supplied stream
    */
