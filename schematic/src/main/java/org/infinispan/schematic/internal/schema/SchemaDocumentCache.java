 ...
 * {@link Validator#validate(Object, String, Document, org.infinispan.schematic.document.Path, Problems) validate} JSON
 * Documents. When a SchemaDocument is needed and has not yet been loaded, this cache finds the corresponding JSON
 * Schema {@link Document documents} from the {@link DocumentLibrary}, and uses a {@link Validator.Factory factory} to
 * create the {@link Validator}.
 * 
...
public class SchemaDocumentCache implements Validator.SchemaDocumentResolver, Serializable {
   ...
   public SchemaDocumentCache(DocumentLibrary jsonSchemaDocuments, String defaultMetaSchemaUri) {
      ...
      this.defaultMetaSchemaUri = defaultMetaSchemaUri != null ? defaultMetaSchemaUri
               : JsonSchema.Version.Latest.CORE_METASCHEMA_URL;
   }
   ...
   public SchemaDocument get(String uri, Problems problems) {
      ...
      try {
         ...
         problems.recordWarning(Paths.path("$schema"), "The URI of the referenced schema '" + uri
                  + "' is not a valid URI");
      }
    ...
    * @param uri
    *           the URI of the schema being read
    * @return true if a SchemaDocument was removed from the cache, or false if this cache did not contain a
    *         SchemaDocument for the supplied URI
    */
