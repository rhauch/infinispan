...
/**
 * A value used to store user's content (often a JSON document or a binary value) and metadata as an entry in a
 * SchematicDb. These values also offer fine-grained locking and serialization of the value.
 * 
...
public interface SchematicEntry extends Cloneable {
   ...
   public static interface FieldName {
      ...
      /**
       * The name of the metadata field used to store the document key. Note that {@value} is also the field name used
       * by <a href="http://tools.ietf.org/html/draft-zyp-json-schema-03#section-5.27">JSON Schema</a>.
       */
      public static final String ID = "id";

      /**
       * The name of the metadata field used to store the reference to the JSON Schema to which the document should
       * conform. Note that {@value} is the field name used by <a
    ...
    * 
    * if it is a Documentrepresented as a Document object. Note that this method will return a non-null value only if
    * the {@link #isContentDocument()} method returns <code>true</code>. {@link #getRawContent()}).
    * 
   ...
   /**
    * Return <code>true</code> if the {@link #getContent()} method would return a Document (or
    * {@link #getContentAsDocument()} would return a non-null value).
    * <p>
   ...
   /**
    * Return <code>true</code> if the {@link #getContent()} method would return a Binary (or
    * {@link #getContentAsBinary()} would return a non-null value).
    * <p>
    * This is equivalent to the following:
    * 
    * <pre>
    ...
    * {@link ContentTypes#JSON application/json}".
    * 
    * @param content
    *           the Document representing the JSON content; may not be null
    * @param metadata
    *           the Document representing the metadata; may be null
    * @param defaultContentType
    *           the value for the MIME type describing the content that should be used if the metadata does not already
    *           contain a "contentType" field, and typically {@link ContentTypes#JSON}, {@link ContentTypes#JSON_SCHEMA}
    ...
    * 
    * @param content
    *           the Binary representation of the content; may not be null
    * @param metadata
    *           the Document representing the metadata; may be null
    * @param defaultContentType
    *           the value for the MIME type describing the content that should be used if the metadata does not already
   ...
   /**
    * Get an {@link EditableDocument editable metadata document}. The client is expected to make these edits within the
    * context of a transaction, and the edits will be saved when the transaction is committed.
    * 
   ...
   /**
    * Get an {@link EditableDocument editable document}, when the content is a {@link #hasDocumentContent() document}.
    * The client is expected to make these edits within the context of a transaction, and the edits will be saved when
