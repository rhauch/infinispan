...
public class JsonPerformanceTest {
   ...
   protected void testReadingJson(final String testData, int numberOfRuns, final boolean getValue,
            final boolean introspectStringValues) throws Exception {
      assert testData != null;
      final JsonReader reader = new JsonReader();
      final AtomicReference<Exception> error = new AtomicReference<Exception>();
      final String key = "key";
      runTest("JsonReader.read(String," + introspectStringValues + ")", numberOfRuns, testData.length(), print,
               new Runnable() {
                  @Override
                  public void run() {
                     try {
   ...
   protected void testReadingJsonWithJsonSimple(final String testData, int numberOfRuns, final boolean getValue)
            throws Exception {
      final org.json.simple.parser.JSONParser parser = new org.json.simple.parser.JSONParser();
      final AtomicReference<Exception> error = new AtomicReference<Exception>();
      final String key = "key";
      runTest("org.json.simple.parser.JSONParser().parse(Reader)", numberOfRuns, testData.length(), print,
               new Runnable() {
                  @Override
                  public void run() {
                     try {
                        org.json.simple.JSONObject obj = (org.json.simple.JSONObject) parser.parse(new StringReader(
                                 testData));
                        if (getValue)
                           obj.get(key);
                     } catch (Exception t) {
   ...
   protected void testReadingJsonThenWritingBsonThenReadingBson(final String testData, int numberOfRuns,
            final boolean getValue, final boolean introspectStringValues) throws Exception {
      assert testData != null;
